...
router.post("/list-store/:id/check-in", authenticateToken, async (req, res) => {
  try {
    ...
    const imgPath =
      car_release_no && String(car_release_no).trim()
        ? `car_release/${String(car_release_no).trim()}/check_in`
        : "check_in";

    const imgCheckInUrl = await saveBase64Image(image_check_in, imgPath);
    const signatureUrl = await saveBase64Image(signature, imgPath);
    ...
    try {
      await query(
        `UPDATE list_store SET status = 'in_progress', start_service_time = ? WHERE store_id = ? AND group_store_id = ?`,
        [dateTime, store_id, group_store_id],
      );
    } catch (eUp) {
      await query(
        `UPDATE list_store SET status = 'in_progress' WHERE store_id = ? AND group_store_id = ?`,
        [store_id, group_store_id],
      );
    }
    ...
    try {
      ...
      if (
        ...
        if (sLat && sLon && cLat && cLon) {
          ...
          try {
            const gpsCfg = await query(
              "SELECT distance_meters FROM gps_distance WHERE distance_code = 'CHECKOUT_MAX' AND is_active = 1 LIMIT 1",
            );
            if (
              gpsCfg &&
              gpsCfg.length > 0 &&
              gpsCfg[0].distance_meters !== undefined
            ) {
      ...
      const imgPath =
        car_release_no && String(car_release_no).trim()
          ? `car_release/${String(car_release_no).trim()}/check_out`
          : "check_out";
      const imgBillUrl = await saveBase64Image(image_bill, imgPath);
      const dateTime = new Date().toISOString().slice(0, 19).replace("T", " ");
      ...
      if (Array.isArray(additional_images) && additional_images.length > 0) {
        for (const imgStr of additional_images) {
          const savedUrl = await saveBase64Image(imgStr, imgPath);
          if (savedUrl) {
            await query(
      ...
      try {
        await query(
          `UPDATE list_store SET status = 'completed', end_service_time = ? ${isOffSite ? ", off_site = 1" : ""} WHERE store_id = ? AND group_store_id = ?`,
          [dateTime, store_id, group_store_id],
        );
      } catch (eUpEnd) {
        await query(
          `UPDATE list_store SET status = 'completed' ${isOffSite ? ", off_site = 1" : ""} WHERE store_id = ? AND group_store_id = ?`,
          [store_id, group_store_id],
        );
      }

      // Update list_store Bypass
      await query(
        `UPDATE list_store SET bypass = 1 WHERE store_id = ? AND group_store_id = ? AND list_id <> ?`,
        [store_id, group_store_id, list_id],
      );
      ...
      if (releaseRow) {
        ...
        let targetCarReleaseId = hasCarReleaseId
          ? releaseRow.car_release_id
          : null;
        let filterCol =
          hasCarReleaseId && targetCarReleaseId
            ? "ls.car_release_id"
            : "ls.group_store_id";
        let filterVal =
          hasCarReleaseId && targetCarReleaseId
            ? targetCarReleaseId
            : releaseRow.group_store_id;

        if (!targetCarReleaseId && releaseRow.group_store_id) {
          const crs = await query(
            `SELECT car_release_id FROM car_release WHERE group_store_id = ? ORDER BY car_release_id DESC LIMIT 1`,
            [releaseRow.group_store_id],
          );
        ...
        if (targetCarReleaseId && filterVal) {
          ...
          if (totals.length > 0) {
            await query(
              ...
              [
                totals[0].total_bills || 0,
                totals[0].sum_amount || 0,
                targetCarReleaseId,
              ],
            );
          }
      ...
      res.json({
        success: true,
        message: isOffSite
          ? "เช็คเอาท์สำเร็จ (แจ้งเตือน: นอกสถานที่ > 300ม.)"
          : "เช็คเอาท์สำเร็จ",
        check_out_id,
        off_site: isOffSite,
      });
...
router.post("/list-store/:id/problem", authenticateToken, async (req, res) => {
  try {
    ...
    const imgPath =
      car_release_no && String(car_release_no).trim()
        ? `car_release/${String(car_release_no).trim()}/problem`
        : "problem";
    ...
    await query(
      `UPDATE list_store SET status = 'problem', end_service_time = ? WHERE list_id = ?`,
      [new Date().toISOString().slice(0, 19).replace("T", " "), list_id],
    );
...
router.patch("/list-store/:id/bypass", authenticateToken, async (req, res) => {
  try {
    ...
    await query(`UPDATE list_store SET bypass = ? WHERE list_id = ?`, [
      bypass ? 1 : 0,
      list_id,
    ]);
