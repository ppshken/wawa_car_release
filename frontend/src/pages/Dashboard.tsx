...
export const Dashboard: React.FC = () => {
  const [activeTab, setActiveTab] = useState<'all' | 'running' | 'returned'>('all');
  ...
  const handleColumnChange = (updated: Record<string, boolean>) => {
    ...
    localStorage.setItem('wawa_dashboard_visible_cols', JSON.stringify(updated));
  };
  ...
  const filteredOperations = operations.filter((op) => {
    const isCompleted = (op.storesDone === op.storesTotal && op.storesTotal > 0) || op.isReturned;
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 pt-2 border-b border-slate-100 pb-4">
        <div>
          ...
          <p className="text-xs text-slate-500 mt-0.5">
            ระบบติดตามสถานะปล่อยขาย ส่งสินค้า รายรับเงินสด/โอน และประสิทธิภาพคนขับ
          </p>
        </div>
        ...
        <div className="flex items-center gap-2.5">
          ...
          <div className="bg-white border border-slate-200/80 rounded-lg px-3 py-1.5 flex items-center gap-2 text-xs text-slate-600 shadow-sm">
            ...
            <select
              ...
              className="bg-transparent focus:outline-none font-medium cursor-pointer text-slate-800"
            >
              <option value="today">วันนี้</option>
              <option value="this_week">7 วันล่าสุด</option>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        <div className="tms-card p-5 hover:border-slate-300 transition-all bg-white rounded-xl border border-slate-200/70 shadow-sm">
          <div className="flex justify-between items-start">
            <span className="text-xs font-medium text-slate-500">ใบปล่อยรถ</span>
          ...
          <div className="mt-3 pt-3 border-t border-slate-100 text-[11px] text-slate-400 flex justify-between">
            <span>คืนรถแล้ว: <strong className="text-slate-700 font-medium">{summary.returned_releases} คัน</strong></span>
          ...
          <div className="mt-3 space-y-1">
            <div className="w-full bg-slate-100 rounded-full h-1.5 overflow-hidden">
              <div
                ...
                style={{ width: `${Math.min(100, summary.stores_completion_percent || 0)}%` }}
              ></div>
            </div>
            <div className="flex justify-between text-[10px] text-slate-400">
              <span>สำเร็จ {summary.stores_completed.toLocaleString()} ร้าน</span>
              <span className="font-semibold text-slate-700">{summary.stores_completion_percent}%</span>
            </div>
          </div>
        </div>
        ...
        <div className="tms-card p-5 hover:border-slate-300 transition-all bg-white rounded-xl border border-slate-200/70 shadow-sm">
          <div className="flex justify-between items-start">
            <span className="text-xs font-medium text-slate-500">ยอดขายรวมจัดเก็บ</span>
            <div className="w-8 h-8 rounded-lg bg-slate-100 text-slate-700 flex items-center justify-center">
          ...
          <div className="mt-3 pt-3 border-t border-slate-100 text-[11px] text-slate-500 flex justify-between">
            <span>สด: <strong className="text-slate-700">{formatCurrency(summary.total_cash)}</strong></span>
            <span>โอน: <strong className="text-slate-700">{formatCurrency(summary.total_transfer_received)}</strong></span>
          </div>
        </div>
        ...
        <div className="tms-card p-5 hover:border-slate-300 transition-all bg-white rounded-xl border border-slate-200/70 shadow-sm">
          ...
          <div className="mt-3 flex items-baseline gap-2">
            <span className="text-2xl font-semibold text-slate-900 tracking-tight">
              {loading ? '-' : summary.pending_transfer_count + summary.off_site_count}
            </span>
          ...
          <div className="mt-3 pt-3 border-t border-slate-100 text-[11px] text-slate-500 flex justify-between">
            <span>ค้างโอน: <strong className="text-slate-700">{summary.pending_transfer_count} รายการ</strong></span>
            <span>Off-site: <strong className="text-slate-700">{summary.off_site_count} จุด</strong></span>
          </div>
        </div>
      </div>
      ...
      <div className="bg-white rounded-xl border border-slate-200/70 p-5 shadow-sm space-y-3">
        <div className="flex items-center justify-between">
          <div>
            ...
            <p className="text-xs text-slate-400 mt-0.5">เปรียบเทียบยอดขายรวม เงินสด และเงินโอนเข้าบัญชีรายวัน</p>
          </div>
          ...
          {dailyStats.length > 0 ? (
            <ResponsiveContainer width="100%" height="100%">
              <AreaChart data={dailyStats} margin={{ top: 10, right: 10, left: 0, bottom: 0 }}>
                ...
                />
                <Area type="monotone" dataKey="cash" name="เงินสด" stroke="#0f172a" strokeWidth={2} fillOpacity={1} fill="url(#colorCash)" />
                <Area type="monotone" dataKey="transfer" name="เงินโอน" stroke="#10b981" strokeWidth={2} fillOpacity={1} fill="url(#colorTransfer)" />
              </AreaChart>
            </ResponsiveContainer>
          ) : (
            ...
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 border-b border-slate-100 pb-3">
              <div>
                ...
                <p className="text-xs text-slate-400 mt-0.5">
                  รายการใบปล่อยรถ ทะเบียน ความคืบหน้าการส่งสินค้า และสถานะทางบัญชี
                </p>
              </div>
                ...
                <tbody className="divide-y divide-slate-200/60 text-slate-800 whitespace-nowrap">
                  {loading ? (
                    ...
                    filteredOperations.map((rel) => {
                      const isCompleted = (rel.storesDone === rel.storesTotal && rel.storesTotal > 0) || rel.isReturned;
                      return (
                        <tr
                            ...
                            <td className="py-2 px-3">
                              <div className="flex items-center gap-1.5 font-medium text-slate-700 bg-slate-100 border border-slate-200 px-2 py-0.5 rounded text-[11px]">
                                <div
                                  ...
                                  style={{ background: rel.groupColor || '#94a3b8' }}
                                />
        ...
        <div className="space-y-4">
          ...
          <div className="bg-white rounded-xl border border-slate-200/70 p-5 space-y-4 shadow-sm">
            <div className="flex items-center justify-between border-b border-slate-100 pb-3">
              ...
              <span className="text-[11px] text-slate-400">ยอดจัดเก็บสูงสุด</span>
            </div>

            <div className="space-y-3">
              {loading ? (
                <div className="py-6 text-center text-xs text-slate-400">กำลังดึงข้อมูล...</div>
              ) : drivers.length === 0 ? (
                <div className="py-6 text-center text-xs text-slate-400">ยังไม่มีข้อมูลคนขับในการจัดส่ง</div>
              ) : (
                drivers.map((d) => (
                  <div key={d.userId} className="flex items-center justify-between text-xs py-1">
                    <div className="flex items-center gap-2.5">
                      ...
                      <div>
                        <div className="font-semibold text-slate-800 flex items-center gap-1">
                          {d.name}
                          {d.active && (
                            <span className="w-1.5 h-1.5 rounded-full bg-emerald-500" title="กำลังวิ่งงานอยู่"></span>
                          )}
                        </div>
                    ...
                    <div className="text-right">
                      <div className="font-semibold text-slate-900">{formatCurrency(d.revenue)}</div>
                    </div>
                  </div>
                ))
              )}
          ...
          <div className="bg-white rounded-xl border border-slate-200/70 p-5 space-y-3 shadow-sm">
            ...
            <div className="pt-2 border-t border-slate-100 flex justify-between text-[11px] text-slate-500">
              <span>โอนค้าง: <strong className="text-amber-600 font-semibold">{summary.pending_transfer_count} รายการ</strong></span>
              <Link to="/reports" className="text-slate-900 font-semibold hover:underline">
                ดูรายงานบัญชี
              </Link>
            </div>
          </div>
