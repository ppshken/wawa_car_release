    ...
    <div className="space-y-1">
      <div className="flex items-center justify-between text-xs">
        <label className="font-bold text-slate-700 flex items-center gap-1.5 text-xs">
          ...
          <span>
            เซ็นลายเซ็นผู้เช็คอิน <span className="text-rose-500">*</span>
          </span>
        </label>
        {hasSigned && (
          <button
...
const SlideToCheckInButton: React.FC<{
  ...
  return (
    <div
      ref={trackRef}
      className={`relative w-full h-12 rounded-xl select-none overflow-hidden flex items-center p-1 border transition-colors shadow-2xs ${
        disabled
    ...
    >
      ...
      <div
        ...
        className={`relative z-10 w-10 h-10 rounded-lg flex items-center justify-center border shadow-xs transition-transform cursor-grab active:cursor-grabbing ${
          disabled
...
export const DeliveryCheckInOutDrawer: React.FC<
  DeliveryCheckInOutModalProps
> = ({
  isOpen,
  onClose,
  ...
  useEffect(() => {
    if (isOpen) {
      api
        .get("/master/problem-types")
        .then((res) => {
          if (
            res.data.success &&
            Array.isArray(res.data.problemTypes) &&
            res.data.problemTypes.length > 0
          ) {
            ...
            const filtered = activeList.filter(
              (n: string) => n !== "+ ระบุปัญหาอื่นๆ (พิมพ์เอง)",
            );
    ...
    const fetchMasterOptions = async () => {
      try {
        ...
        if (
          ...
          payRes.value.data.success &&
          Array.isArray(payRes.value.data.payments)
        ) {
        ...
        if (
          gpsRes.status === "fulfilled" &&
          gpsRes.value.data.success &&
          Array.isArray(gpsRes.value.data.distances)
        ) {
          const checkinRule = gpsRes.value.data.distances.find(
            (d: any) =>
              d.distance_code === "CHECKIN_RADIUS" &&
              (d.is_active === 1 ||
                d.is_active === true ||
          ...
          if (
            checkinRule &&
            checkinRule.distance_meters !== undefined &&
            checkinRule.distance_meters !== null
          ) {
  ...
  const requestGpsLocation = () => {
    ...
    navigator.geolocation.getCurrentPosition(
      ...
      (err) => {
        ...
        setLocationError(
          err.message ||
            "ไม่สามารถดึงตำแหน่ง GPS ได้ กรุณาเปิด Location Service",
        );
      },
  ...
  const isCompleted =
    storeItem.status === "completed" || Boolean(storeItem.check_out_id);
  const isProblem =
    storeItem.status === "problem" || Boolean(storeItem.problem_id);
  ...
  const handleSubmitProblem = async () => {
    ...
    try {
      ...
      showError(
        err?.response?.data?.message || "เกิดข้อผิดพลาดในการบันทึกปัญหา",
      );
    } finally {
  ...
  const startTimeVal =
    storeItem.start_service_time ||
    storeItem.date_time_check_in ||
    storeItem.check_in_time;
  const endTimeVal =
    storeItem.end_service_time ||
        ...
        <div className="bg-white mb-2 mt-2 flex flex-wrap items-center justify-between gap-1.5 text-xs shadow-2xs">
          <div className="flex items-center gap-1.5">
            ...
            {isLocating ? (
              ...
              <span
                className={`font-mono font-bold px-2 py-0.2 rounded text-xs ${
                  distanceMeters <= maxDistanceMeters
                  ...
                  {distanceMeters !== null &&
                    ...
                    distanceMeters > maxDistanceMeters && (
                      <div className="bg-rose-50 border border-rose-200 rounded-lg p-2 text-xs text-rose-700 flex items-center gap-1.5 font-semibold">
                        ...
                        <span>
                          ไม่สามารถเช็คอินได้เนื่องจากระยะห่าง GPS (
                          {distanceMeters} ม.) เกินกว่าที่กำหนด (≤{" "}
                          {maxDistanceMeters} ม.)
                        </span>
                      </div>
                    )}
              ...
              <div className="divide-y divide-slate-100 text-xs">
                ...
                <div className="py-1.5  flex items-center justify-between gap-2 bg-white">
                  ...
                  <span className="font-bold text-slate-800 text-right text-xs">
                    {String(storeItem.priority || "").toLowerCase() ===
                      "high" || storeItem.priority === "สูง"
                      ? "สูง"
                      : String(storeItem.priority || "").toLowerCase() ===
                            "low" || storeItem.priority === "ต่ำ"
                        ? "ต่ำ"
                        : "กลาง"}
                  </span>
                </div>
                ...
                <div className="py-1.5  flex items-center justify-between gap-2 bg-white">
                  ...
                  <span className="font-mono font-bold text-slate-900 text-right text-xs">
                    {formatTimeString(
                      storeItem.scheduled_time || storeItem.scheduledTime,
                    )}
                  </span>
                </div>
                  ...
                  {isCompleted && (
                    <div className="flex gap-2.5">
                      ...
                      <div className="flex-1 space-y-2 border-t border-slate-200 pt-2 text-xs">
                        ...
                        {/* Check-out Photos */}
                        {(storeItem.image_bill ||
                          (Array.isArray(storeItem.checkout_images) &&
                            storeItem.checkout_images.length > 0)) && (
                          <div className="flex flex-wrap items-center gap-2 pt-0.5">
                            {(() => {
                              ...
                              if (Array.isArray(storeItem.checkout_images)) {
                                storeItem.checkout_images.forEach(
                              ...
                              return checkoutPhotoList.map((photo, idx) => (
                                <div
                                  key={idx}
                                  onClick={() =>
                                    openLightbox(checkoutPhotoList, idx)
                                  }
                                  className="shrink-0 group flex flex-col items-center gap-0.5 cursor-pointer"
                                  title="คลิกเพื่อขยายรูปภาพ"
                                >
                              ...
                              <tbody className="divide-y divide-slate-100 text-xs">
                                ...
                                <tr>
                                  ...
                                  <td className="py-1.5 px-3 font-mono font-extrabold text-emerald-700 text-xs">
                                    {(
                                      storeItem.amount ||
                                      Number(storeItem.cash || 0) +
                                        Number(storeItem.transfer || 0)
                                    ).toLocaleString()}{" "}
                                    ฿
                                  </td>
                                </tr>
                      ...
                      <div className="flex-1 space-y-1.5 bg-rose-50/50 border border-rose-200 rounded-lg p-2.5 text-rose-950">
                        <div className="flex items-center justify-between">
                          <span className="font-bold text-xs text-rose-950 flex items-center gap-1.5">
                            <span>
                              ติดปัญหา:{" "}
                              {storeItem.problem_name || "ติดปัญหาจัดส่ง"}
                            </span>
                          </span>
                          <span className="font-mono text-[11px] text-slate-500">
                        ...
                        {/* Problem Photos */}
                        {Array.isArray(storeItem.problem_images) &&
                          storeItem.problem_images.length > 0 && (
                            <div className="flex flex-wrap items-center gap-2 pt-0.5">
                              {storeItem.problem_images.map(
                                (imgUrl: string, idx: number) => (
                                  <a
                                    key={idx}
          ...
          <div className="space-y-3">
            ...
            <button
              ...
              className={`w-full py-3.5 rounded-xl font-bold text-xs flex items-center justify-center gap-2 shadow-sm transition-all ${
                isSubmittingCheckout || checkOutImgs.length === 0
                  ? "bg-slate-200 text-slate-400 cursor-not-allowed shadow-none"
